...
final class MvtBonsValideController extends AbstractController
{
                    ...
                    $sql = sprintf(
                        "INSERT INTO MVT_BONS_VALIDE 
                            (NUMMVT, D_BONS, NUMSEM, NBRSS, NBRSS0, NBRSS1, NBRSS2,
                             ...
                             TXAMIN, TXAMAX, TXMP)
                         VALUES 
                            (seq_MVT_BONS_VALIDE.NEXTVAL, '%s', 6, NULL, NULL, NULL, NULL,
                             %s, %s, %s, NULL, %s, NULL,
                    ...
                    foreach ($insertedIds as $nummvt) {
                        $sqlHistorique = sprintf(
                            "INSERT INTO MVT_BONS_VALIDE_HISTORIQUE 
                             ...
                             VALUES 
                                (seq_MVT_BONS_VALIDE_HISTORIQUE.NEXTVAL, %d, TO_DATE('%s', 'YYYY-MM-DD HH24:MI:SS'), '%s')",
                            $nummvt,
                            $currentDate,
    ...
    public function edit(Request $request, MvtBonsValide $mvtBonsValide, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($request->getMethod() === 'GET') {
            $form->get('D_BONS')->setData(
                $dBonsCurrent ? \DateTime::createFromFormat('Y-m-d', $dBonsCurrent) : null
            );
            
            // Pré-remplir les taux
            if (isset($currentData[1])) {
                $form->get('taux_4_semaines')->setData(
                    $transformer->toFloat($currentData[1]['TXMP'] ?? null)
                );
            }
            
            if (isset($currentData[2])) {
                $form->get('taux_12_semaines')->setData(
                    $transformer->toFloat($currentData[2]['TXMP'] ?? null)
                );
            }
            
            if (isset($currentData[3])) {
                $form->get('taux_24_semaines')->setData(
                    $transformer->toFloat($currentData[3]['TXMP'] ?? null)
                );
            }
            
            if (isset($currentData[4])) {
                $form->get('taux_26_semaines')->setData(
                    $transformer->toFloat($currentData[4]['TXMP'] ?? null)
                );
            }
            
            if (isset($currentData[5])) {
                $form->get('taux_52_semaines')->setData(
                    $transformer->toFloat($currentData[5]['TXMP'] ?? null)
                );
            }
            
            if (isset($currentData[6])) {
                $form->get('montant_annonce')->setData(
                    $transformer->toFloat($currentData[6]['MAN'] ?? null)
                );
            
                $form->get('montant_soumis')->setData(
                    $transformer->toFloat($currentData[6]['MSM1'] ?? null)
                );
            
                $form->get('montant_adjuge')->setData(
                    $transformer->toFloat($currentData[6]['MAD1'] ?? null)
                );
            }
        }
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // Récupérer la nouvelle date des bons
            $dBons = $form->get('D_BONS')->getData();
            $dBonsStr = $dBons instanceof \DateTime ? $dBons->format('Y-m-d') : null;
            ...
            try {
                ...
                // Supprimer tous les enregistrements avec l'ancienne date
                $sqlDelete = sprintf(
                    "DELETE FROM MVT_BONS_VALIDE WHERE D_BONS = '%s'",
                    $dBonsCurrent
                );
                $conn->executeStatement($sqlDelete);
    
                // ========== INSERTION DES NOUVEAUX TAUX MOYENS PONDÉRÉS ==========
                foreach ($tauxMapping as $numSem => $tauxValue) {
                    if ($tauxValue !== null && $tauxValue !== '') {
                        $txmp = number_format((float)$tauxValue, 2, '.', '');
